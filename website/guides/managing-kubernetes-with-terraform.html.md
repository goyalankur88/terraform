...
orchestration with Terraform presents a few benefits.
 ...
 - HCL - same language for both lower layer with underlying
    infrastructure (compute) and scheduling layer
 ...
 - synchronous feedback - While asynchronous behaviour is often useful,
    ...
    hence you can't create DNS record pointing to it.
 - graph of relationships - Terraform understands relationships between resources
    which may help in scheduling - e.g. if PVC claims space from a particular PV,
    Terraform won't even attempt to create PVC if the creation of PV has failed.
...
The bread and butter of any K8S app is a Pod. Pod consists of one or more
containers which are scheduled on cluster nodes based on cpu or memory
being available.
...
high availability. Adding labels will allow K8S to find any pod (instance)
for the purpose of forwarding the traffic to the exposed port.
...
The simplest way to expose your application to users is via Service.
...
The Kubernetes UI also provides you feedback so you can
see both the pod and the service there once they're scheduled.
...
avoiding under-provisioning or over-provisioning resulting in either unused resources
or lack of resources (causing the app to crash on OOM).
